    ...
    def crawl(self) -> dict[str, str]:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            while urls_to_visit and len(self.visited_urls) < self.max_pages:
                ...
                logging.info(f"[{len(self.visited_urls)}/{self.max_pages}] Scraping: {url}")
        ...
        logging.info(f"Crawling complete. Scraped {len(self.scraped_content)} pages with content.")
    ...
    def run_full_process(self):
        try:
            ...
            if mode == "scrape":
                ...
                scraper = WebsiteScraper(url, max_pages=limit if limit > 0 else 20)
