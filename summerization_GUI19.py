...
class WebsiteScraper:
    def __init__(self, base_url: str, max_pages: int, user_agent: str, request_timeout: int, rate_limit_delay: float, gemini_api_key: Optional[str] = None, deep_crawl: bool = False, crawl_depth: int = 0):
        ...
        if self.gemini_api_key and DEPENDENCIES_AVAILABLE:
            try:
                self.llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", google_api_key=self.gemini_api_key, temperature=0.0)
...
class AdvancedScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Research & Study Guide Generator v2.8 (Multimodal + Hugo)")
    ...
    def create_widgets(self):
        ...
        source_tab, research_tab, ai_tab, prompt_tab, integrations_tab = (ttk.Frame(settings_notebook, padding=15) for _ in range(5))
        ...
        self.create_integrations_tab(integrations_tab)
        ...
        settings_notebook.add(integrations_tab, text="🔩 Integrations")
    ...
    def create_integrations_tab(self, parent):
        ...
        ttk.Label(hugo_frame, text="Hugo Project Directory:").pack(fill=tk.X, anchor='w')
        ttk.Label(hugo_frame, text="(Select any folder inside your Hugo project)", font=("Segoe UI", 8)).pack(fill=tk.X, anchor='w', pady=(0,2))
        dir_frame = ttk.Frame(hugo_frame)
        dir_frame.pack(fill=tk.X, pady=(0, 8))
        ttk.Entry(dir_frame, textvariable=self.hugo_dir_var, font=("Segoe UI", 9)).pack(side=tk.LEFT, expand=True, fill=tk.X)
        ttk.Button(dir_frame, text="📁", width=3, command=self.browse_hugo_dir).pack(side=tk.RIGHT, padx=(5, 0))
    ...
    def run_youtube_video_analysis(self):
        ...
        try:
            ...
            # Strip metadata for Hugo as it will have its own
    ...
    def browse_hugo_dir(self):
        filepath = filedialog.askdirectory(title="Select Your Hugo Project Folder")
        if not filepath:
            return
        # Find the actual root and update the variable for a better user experience
        found_root = self._find_hugo_root(Path(filepath))
        if found_root:
            self.hugo_dir_var.set(str(found_root))
            logging.info(f"✅ Automatically verified Hugo project root: {found_root}")
        else:
            # Keep the user's selection but warn them it might be wrong
            self.hugo_dir_var.set(filepath)
            messagebox.showwarning("Hugo Warning", "Could not automatically verify the selected folder as a Hugo project root. Please ensure it is correct.")
    ...
    def load_initial_settings(self):
        ...
        if self.config:
            ...
            hugo_settings = self.config.get('hugo', {})
    ...
    def save_notes(self):
        ...
        try:
            # When saving manually, include the metadata header
            full_content_to_save = self.final_notes_content
            if not self.final_notes_content.strip().startswith("GENERATION METADATA"):
                 full_content_to_save = f"# Study Guide\n\n{self.final_notes_content}"

            with open(filepath, "w", encoding="utf-8") as f: f.write(full_content_to_save)
...
def sanitize_filename(self, text: str) -> str:
    """Converts a string into a clean filename."""
...
def _find_hugo_root(self, start_path: Path) -> Optional[Path]:
    """Find Hugo project root by looking for config files and content directory."""
    ...
    config_files = ["hugo.toml", "hugo.yaml", "hugo.json", "config.toml", "config.yaml", "config.json"]
    max_levels = 10  # Prevent infinite loops
    
    for _ in range(max_levels):
        # Check if this looks like a Hugo root
        has_content = (current_path / "content").is_dir()
        has_config = any((current_path / config).is_file() for config in config_files)
        
        if has_content and has_config:
            logging.info(f"✅ Found Hugo project root: {current_path}")
            return current_path
    ...
    logging.warning(f"❌ No Hugo project found starting from {start_path}")
    return None

def handle_hugo_integration(self):
    """Simplified Hugo integration that creates a new post."""
    self.progress_var.set("Creating Hugo post...")
    logging.info("🔩 Starting Hugo integration...")
    
    # Validate Hugo directory
    hugo_dir_str = self.hugo_dir_var.get().strip()
    if not hugo_dir_str:
        messagebox.showerror("Hugo Error", "Please specify a Hugo project directory.")
        self.save_button.config(state=tk.NORMAL)
        return
    ...
    if not hugo_root:
        messagebox.showerror("Hugo Error", 
            f"Could not find a Hugo project at:\n{hugo_dir_str}\n\n"
            f"Make sure the directory contains:\n"
            f"• A 'content' folder\n"
            f"• A Hugo config file (hugo.toml, config.toml, etc.)")
        self.save_button.config(state=tk.NORMAL)
        return
    
    # Check Hugo command availability
    if not shutil.which("hugo"):
        messagebox.showerror("Hugo Error", 
            "Hugo command not found. Please install Hugo:\n"
    ...
    try:
        ...
        content_lines = self.final_notes_content.split('\n')
        clean_content = []
        skip_metadata = False
        
        for line in content_lines:
            if line.startswith('GENERATION METADATA:'):
                skip_metadata = True
                continue
            elif skip_metadata and line.startswith('='):
                skip_metadata = False
                continue
        ...
        if title_match:
            ...
            title = f"Study Guide {datetime.now().strftime('%Y-%m-%d')}"
        ...
        post_path = f"{content_subdir}/{filename}.md"
        full_path = hugo_root / "content" / content_subdir / f"{filename}.md"
        ...
        if full_path.exists():
            response = messagebox.askyesno("File Exists", 
                f"Post already exists:\n{filename}.md\n\nOverwrite?")
            if not response:
                logging.info("Hugo integration cancelled - file exists")
                self.save_button.config(state=tk.NORMAL)
                return
        
        # Use Hugo to create the post
        logging.info(f"Creating Hugo post: {post_path}")
        cmd = ["hugo", "new", post_path]
        ...
        logging.info(f"Hugo command output: {result.stdout.strip()}")
        
        # Write our content to the created file
        if full_path.exists():
            # Read existing frontmatter
            with open(full_path, 'r', encoding='utf-8') as f:
                existing_content = f.read()
            
            # Find where frontmatter ends (look for closing ---)
            if existing_content.startswith('---'):
                parts = existing_content.split('---', 2)
                if len(parts) >= 3:
                    frontmatter = f"---{parts[1]}---"
                    # Replace the content after frontmatter
                    new_content = f"{frontmatter}\n\n{content_body}"
                else:
                    # No proper frontmatter found, append to existing
                    new_content = f"{existing_content}\n\n{content_body}"
            else:
                # No frontmatter detected, just overwrite
                new_content = content_body
        else:
            # File wasn't created by Hugo, create it ourselves
            new_content = f"""---
title: "{title}"
...
{content_body}"""
        ...
        logging.info(f"✅ Successfully created Hugo post: {full_path}")
        
        # Offer to open the file location
        if messagebox.askyesno("Hugo Success", 
            f"Successfully created Hugo post:\n{full_path.name}\n\n"
            f"Would you like to open the containing folder?"):
            
            if sys.platform == "win32":
                os.startfile(full_path.parent)
            elif sys.platform == "darwin":
                subprocess.run(["open", full_path.parent])
            else:
                subprocess.run(["xdg-open", full_path.parent])
        
    except subprocess.CalledProcessError as e:
        error_msg = f"Hugo command failed:\n{e.stderr.strip() if e.stderr else str(e)}"
        logging.error(f"❌ Hugo error: {error_msg}")
        messagebox.showerror("Hugo Error", error_msg)
        self.save_button.config(state=tk.NORMAL)
        
    except Exception as e:
        logging.error(f"❌ Hugo integration failed: {e}")
        messagebox.showerror("Hugo Integration Error", f"Unexpected error:\n{str(e)}")
        self.save_button.config(state=tk.NORMAL) 
