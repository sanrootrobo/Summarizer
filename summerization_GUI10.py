...
import webbrowser
import tempfile  # <-- CORRECTED: Added the missing import here
import sys
...
class WebsiteScraper:
    ...
    def crawl(self, additional_urls: Optional[List[str]] = None) -> dict[str, str]:
        ...
        processed_count = 0
        while urls_to_visit:
            if self.max_pages > 0 and len(self.scraped_content) >= self.max_pages:
...
class AdvancedScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Research & Study Guide Generator v2.1")
    ...
    def create_source_tab(self, parent):
        ...
        self.scraper_frame = ttk.LabelFrame(parent, text="🕷️ Initial Crawl Settings", padding=10)
        ...
        # CORRECTED: Clarified UI label for initial crawl limit
        ttk.Label(self.scraper_frame, text="Max pages to crawl on this domain:").pack(fill=tk.X, anchor='w')
    ...
    def create_research_tab(self, parent):
        ...
        ttk.Checkbutton(master_frame, text="🔬 Enable AI-Powered Research (Beta)", variable=self.research_enabled_var, command=self.toggle_research_panel).pack(anchor=tk.W)
        ...
        # CORRECTED: Clarified UI label for research crawl limit
    ...
    def run_full_process(self):
        try:
            ...
            self.progress_var.set("Validating inputs..."); logging.info("🚀 Starting Enhanced Research & Study Guide Generation...")
            ...
            if self.research_enabled_var.get():
                ...
                if self.web_research_enabled_var.get():
                    ...
                    if research_urls:
                        # CORRECTED: Use the explicit research page limit
                        ...
                        # Use a new scraper instance for the research phase
