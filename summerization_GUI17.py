...
class AdvancedScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Research & Study Guide Generator v2.5 (Audio Synthesis)")
    ...
    def create_widgets(self):
        ...
        self.audio_button = ttk.Button(button_frame, text="🔊 Synthesize Audio", command=self.start_audio_synthesis_thread, state=tk.DISABLED)
        self.audio_button.pack(fill=tk.X, ipady=2, pady=(5,0))
    ...
    def run_full_process(self):
        try:
            ...
            self.root.after(0, lambda: self.audio_button.config(state=tk.NORMAL))
    ...
    def _clean_markdown_for_speech(self, text: str) -> str:
        """Removes common Markdown syntax to produce cleaner text for TTS."""
        # Remove headers (e.g., #, ##, ###)
        text = re.sub(r'^#+\s+', '', text, flags=re.MULTILINE)
        # Remove bold, italic, and inline code markers
        ...
        # Remove blockquotes
        text = re.sub(r'^\s*>\s?', '', text, flags=re.MULTILINE)
        # Remove image and link syntax but keep the link text
        text = re.sub(r'!\[(.*?)\]\(.*?\)', r'\1', text) # Keep alt text for images
        text = re.sub(r'\[(.*?)\]\(.*?\)', r'\1', text) # Keep text for links
        # Collapse multiple newlines into a single paragraph break
        text = re.sub(r'\n{2,}', '\n\n', text)
        return text.strip()

    def start_audio_synthesis_thread(self):
        """Starts the audio synthesis process in a separate thread."""
        if self.is_processing:
            messagebox.showwarning("Busy", "Another process is running. Please wait.")
            return
        if not self.final_notes_content:
            messagebox.showinfo("No Content", "There is no study guide content to synthesize.")
            return

        self.is_processing = True
        self.progress_var.set("Starting audio synthesis...")
        self.audio_button.config(state=tk.DISABLED)
        self.start_button.config(state=tk.DISABLED)

        threading.Thread(target=self.synthesize_audio, daemon=True).start()

    def synthesize_audio(self):
        """Generates speech from the final notes and saves it as an MP3 file."""
        try:
            logging.info("🔊 Starting audio synthesis process...")
            self.progress_var.set("Configuring audio API...")
            ...
            source_name_raw = "audio_guide" # Generic name for audio file
            if self.input_mode_var.get() == "scrape":
                source_name_raw = urlparse(self.url_var.get()).netloc or "website_audio"
            elif self.input_mode_var.get() in ["youtube_video", "upload"]:
                source_name_raw = "multimodal_audio"

            source_name = re.sub(r'[^a-zA-Z0-9_-]', '', source_name_raw)
            initial_filename = f"{source_name}_{datetime.now().strftime('%Y%m%d_%H%M')}.mp3"

            # This must run in the main thread, so we use 'after' to schedule it
            ...
            if not filepath:
                logging.info("Audio synthesis cancelled by user.")
                return # Exit if user cancels

            # 3. Clean and prepare the text
            logging.info("Cleaning Markdown for speech synthesis...")
            self.progress_var.set("Preparing text...")
            cleaned_text = self._clean_markdown_for_speech(self.final_notes_content)

            if len(cleaned_text) < 10:
                 logging.warning("Not enough text to synthesize after cleaning.")
                 messagebox.showwarning("No Text", "After cleaning, there was not enough content to create audio.")
                 return

            # 4. Call the Gemini Text-to-Speech API
            logging.info("Synthesizing audio... This may take a moment.")
            self.progress_var.set("Synthesizing audio...")

            # CORRECTED USAGE:
            # Using a high-quality, human-like female voice model.
            # This requires the updated 'google-generativeai' library.
            model = genai.GenerativeModel('models/text-to-speech-001') # Specify the TTS model
            response = model.generate_content(
                cleaned_text,
                # voice='en-US-Neural2-F' # This is an example of a specific high quality voice
            )
            
            # The API documentation now recommends instantiating the text-to-speech model.
            # The top-level function `genai.text_to_speech` is the newest, most direct way.
            # If `pip install --upgrade google-generativeai` was successful, this will work.
            response = genai.text_to_speech(
                model='models/text-to-speech',
                text=cleaned_text,
                voice='en-US-Neural2-F' # A high-quality, natural-sounding female voice
            )

            # 5. Save the audio content
            logging.info(f"Saving audio to {filepath}")
            self.progress_var.set("Saving audio file...")
            with open(filepath, 'wb') as f:
                f.write(response['audio_content'])

            logging.info("✅ Audio synthesis complete.")
            self.progress_var.set("Audio saved successfully!")
            if messagebox.askyesno("Success", f"Audio successfully saved to:\n{filepath}\n\nDo you want to play the file?"):
                try:
                    ...
                    logging.error(f"Failed to open audio file automatically: {e}")
                    messagebox.showwarning("Playback Error", "Could not open the audio file automatically. Please open it manually.")

        except AttributeError:
             logging.error("AttributeError: 'google.generativeai' likely outdated.", exc_info=True)
             messagebox.showerror("Outdated Library", "Your 'google-generativeai' library is out of date.\nPlease update it by running:\npip install --upgrade google-generativeai")
        except Exception as e:
            logging.error(f"❌ An error occurred during audio synthesis: {e}", exc_info=True)
            messagebox.showerror("Synthesis Error", f"An unexpected error occurred: {e}")
        finally:
            self.is_processing = False
            # Re-enable buttons on the main thread
            self.root.after(0, lambda: self.start_button.config(state=tk.NORMAL))
            if self.final_notes_content:
                self.root.after(0, lambda: self.audio_button.config(state=tk.NORMAL))
                self.root.after(0, lambda: self.save_button.config(state=tk.NORMAL))
