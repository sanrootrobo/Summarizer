...
class WebsiteScraper:
    """ An intelligent web scraper that uses an LLM to select relevant links when crawling. """
    def __init__(self, base_url: str, max_pages: int, user_agent: str, request_timeout: int, rate_limit_delay: float, gemini_api_key: Optional[str] = None):
    ...
    def _get_crawl_topic(self, initial_content: str) -> str:
        """Uses the LLM to determine the central topic of the initial page."""
        if not self.llm:
            return "general information"
        try:
            prompt = ChatPromptTemplate.from_template(
                "You are an expert web crawler. Summarize the main topic of the following text in a short, concise phrase to guide your crawling. Example topics: 'asynchronous programming in Python', 'React state management libraries', 'best practices for container security'.\n\n"
                "TEXT CONTENT:\n---\n{content}\n---\n\nMain Topic:"
            )
    ...
    def _select_relevant_urls_with_llm(self, links_with_text: List[Tuple[str, str]], limit: int) -> List[str]:
        """Uses LLM to select the most relevant URLs based on the crawl topic."""
        # Fallback to basic selection if LLM is not available or there's no topic
        if not self.llm or not self.crawl_topic:
        ...
        try:
            prompt = ChatPromptTemplate.from_template(
                "You are an intelligent web crawler assistant. Your goal is to find the most informative pages related to a specific topic.\n"
                "Based on the **Main Topic**, select the **{limit}** most relevant URLs from the list below. The link text provides clues about the page's content.\n"
                "Prioritize links that suggest tutorials, guides, in-depth articles, or core features. Avoid links like 'contact', 'about', 'pricing', 'login', or 'privacy policy' unless they are directly relevant to the topic.\n\n"
                "**Main Topic:** {topic}\n\n"
                "**Candidate Links:**\n{links}\n\n"
                "Respond with ONLY a JSON-formatted list of the selected URL strings. Example: [\"https://.../url1\", \"https://.../url2\"]"
            )
            chain = prompt | self.llm | StrOutputParser()
            response = chain.invoke({
                "topic": self.crawl_topic,
            ...
            if json_match:
                ...
                if isinstance(selected_urls, list) and all(isinstance(i, str) for i in selected_urls):
                    return selected_urls[:limit] # Respect the limit
            
            logging.warning("⚠️ LLM response for URL selection was not valid JSON. Falling back to basic selection.")

        except Exception as e:
            logging.error(f"❌ LLM URL selection failed: {e}. Falling back to basic selection.")
    ...
    def _parse_content_and_links(self, html: str, page_url: str) -> tuple[str, list[tuple[str, str]]]:
        ...
        for a in soup.find_all('a', href=True):
            href = a['href']
            # Ignore anchors on the same page and mailto/tel links
            if href.startswith('#') or href.startswith('mailto:') or href.startswith('tel:'):
                continue
            
            full_url = urljoin(page_url, href).split('#')[0]
            # Ensure it's a valid http/https URL within the same domain
            if urlparse(full_url).scheme in ['http', 'https'] and urlparse(full_url).netloc == self.domain:
                anchor_text = a.get_text(strip=True)
                # Only consider links with meaningful text to provide context to the LLM
    ...
    def crawl(self, additional_urls: Optional[List[str]] = None) -> dict[str, str]:
        ...
        while urls_to_visit and len(self.scraped_content) < self.max_pages:
            ...
            logging.info(f"[{len(self.scraped_content) + 1}/{self.max_pages}] {log_prefix}: {url}")
            
            html = self._get_page_content(url)
            if html:
                text, new_links_with_text = self._parse_content_and_links(html, url)
                
                # If this is the very first page, determine the crawl topic
                if not self.crawl_topic and text and self.llm:
                ...
                # Logic for adding new URLs to the queue
                if not is_external and new_links_with_text:
                    remaining_capacity = self.max_pages - len(self.scraped_content)
                    if remaining_capacity <= 0: continue

                    unvisited_links = [(u, t) for u, t in new_links_with_text if u not in self.visited_urls and u not in urls_to_visit]
                    
                    # If LLM is active and we have more links than we can crawl, get selective
                    if self.llm and self.crawl_topic and len(unvisited_links) > remaining_capacity:
                        logging.info(f"🧠 Found {len(unvisited_links)} links, exceeds capacity ({remaining_capacity}). Using LLM to select the most relevant ones...")
                        ...
                        urls_to_visit.update(selected_urls)
                    else:
                        # Otherwise, just add them until capacity is reached (basic method)
                        urls_to_visit.update([u for u, t in unvisited_links[:remaining_capacity]])
...
class AdvancedScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Research & Study Guide Generator v2.2 (Multimodal)")
        ...
        self.limit_var = tk.IntVar(value=5) # Default initial crawl limit
        ...
        self.research_pages_var = tk.IntVar(value=5) # Default research crawl limit
    ...
    def create_source_tab(self, parent):
        ...
        ttk.Radiobutton(mode_frame, text="🌐 Web Scraper (Context-Aware)", variable=self.input_mode_var, value="scrape", command=self.toggle_input_mode).pack(anchor=tk.W)
        ...
        # This frame contains settings specific to web scraping, to be shown/hidden
        self.scraper_options_frame = ttk.Frame(self.url_input_frame)
        self.scraper_options_frame.pack(fill=tk.X, pady=(0, 0))
        ttk.Label(self.scraper_options_frame, text="Max pages to crawl (triggers intelligent selection):").pack(fill=tk.X, anchor='w')
    ...
    def run_full_process(self):
        ...
        try:
            ...
            if mode == "scrape":
                ...
                # MODIFICATION: Pass the Gemini API key to the scraper
                scraper = WebsiteScraper(url, limit, user_agent, 15, delay, gemini_api_key=api_key)
    ...
    def toggle_input_mode(self):
        ...
        is_video_mode = (mode == "youtube_video")

        # Handle URL input frame visibility
        if is_url_mode:
            self._set_child_widgets_state(self.url_input_frame, tk.NORMAL)
            if is_scrape_mode:
                self.url_input_frame.config(text="🕷️ Web Scraper Input")
                self.scraper_options_frame.pack(fill=tk.X, pady=(2, 0)) # Show scraper options
                self.research_checkbutton.config(state=tk.NORMAL) # Enable research
                self.toggle_research_panel()
            else: # is_video_mode
                self.url_input_frame.config(text="🎬 YouTube Video Input")
                self.scraper_options_frame.pack_forget() # Hide scraper options
                self.research_enabled_var.set(False) # Force disable research for this mode
                self.research_checkbutton.config(state=tk.DISABLED)
                self.toggle_research_panel()
        else:
            self._set_child_widgets_state(self.url_input_frame, tk.DISABLED)
