...
# Make sure to install these: pip install beautifulsoup4 langchain-google-genai google-generativeai PyMuPDF python-docx langchain-community langchain faiss-cpu ollama requests
...
class EnhancedNoteGenerator:
    ...
    def generate_comprehensive_notes(self, source_data: dict[str, str], source_name: str) -> str:
        ...
        logging.info(f"Generating notes from {len(source_data)} sources using Gemini...")
        full_content = ""
...
# --- NEW: Replaces chat functionality with single-pass generation ---
class OllamaRAGGenerator:
    """
    Handles the RAG generation process using Ollama to produce a complete document.
    """
    def __init__(self, file_paths: list[str], ollama_model: str):
        self.file_paths = file_paths
        self.ollama_model = ollama_model
        self.qa_chain = None

    def _load_documents(self):
        """Loads various document types from the file paths."""
        docs = []
        for file_path in self.file_paths:
            try:
                if file_path.lower().endswith(".pdf"):
                    ...
                    docs.extend(loader.load())
                logging.info(f"Successfully loaded document: {Path(file_path).name}")
            except Exception as e:
                logging.error(f"Failed to load {file_path}: {e}")
        return docs

    def _setup_pipeline(self, prompt_template: str) -> bool:
        """Creates the full RAG pipeline and stores it."""
        try:
            # 1. Load documents
            logging.info("RAG: Loading documents...")
            documents = self._load_documents()
            if not documents:
                logging.error("RAG Error: Could not load any documents.")
                return False

            # 2. Split documents
            logging.info("RAG: Splitting documents into chunks...")
            text_splitter = RecursiveCharacterTextSplitter(chunk_size=1500, chunk_overlap=200)
            texts = text_splitter.split_documents(documents)
            if not texts:
                logging.error("RAG Error: Could not split documents into text chunks.")
                return False

            # 3. Create embeddings and vector store
            logging.info("RAG: Creating embeddings with 'nomic-embed-text' and building FAISS vector store...")
            embeddings = OllamaEmbeddings(model="nomic-embed-text")
            vectorstore = FAISS.from_documents(texts, embeddings)
            logging.info("RAG: Vector store created successfully.")

            # 4. Set up the prompt template from the main prompt editor
            # The {context} and {question} placeholders are required by RetrievalQA
            full_prompt_template = f"""
            {prompt_template}

            Use the following context to perform the task described above.
            Context: {{context}}

            Question: {{question}}
            Helpful Answer:"""
            QA_CHAIN_PROMPT = PromptTemplate.from_template(full_prompt_template)

            # 5. Create the RAG chain
            logging.info(f"RAG: Initializing RetrievalQA chain with Ollama model: '{self.ollama_model}'...")
            self.qa_chain = RetrievalQA.from_chain_type(
                Ollama(model=self.ollama_model),
                retriever=vectorstore.as_retriever(),
                chain_type_kwargs={"prompt": QA_CHAIN_PROMPT}
            )
            logging.info("RAG: Pipeline created successfully.")
            return True

        except Exception as e:
            logging.error(f"An error occurred during RAG pipeline creation: {e}", exc_info=True)
            messagebox.showerror("RAG Pipeline Error", f"Failed to build the RAG pipeline.\n\nError: {e}\n\nPlease ensure Ollama is running and the specified models are available.")
            return False

    def generate(self, main_prompt: str) -> str:
        """Sets up the pipeline and generates the full notes based on the main prompt."""
        if not self._setup_pipeline(main_prompt):
            return "# RAG Setup Failed\n\nCould not initialize the RAG pipeline. Check the logs for details."
        
        if not self.qa_chain:
            return "Error: RAG chain is not initialized."
            
        try:
            # The "query" is the user's high-level instruction from the prompt file.
            logging.info(f"RAG Generation: Using the main prompt as the guiding instruction.")
            result = self.qa_chain.invoke({"query": main_prompt})
            logging.info(f"RAG generation complete.")
...
class ScraperApp:
    def __init__(self, root):
        ...
        self.root.geometry("900x750") # Reverted to original height

        self.input_mode_var = tk.StringVar(value="scrape")
        self.processing_model_var = tk.StringVar(value="Gemini Summarization")
    ...
    def create_widgets(self):
        ...
        ttk.Label(self.upload_frame, text="Processing Engine:").pack(anchor=tk.W)
        self.processing_model_dropdown = ttk.Combobox(self.upload_frame, textvariable=self.processing_model_var, values=["Gemini Summarization", "Ollama RAG"], state="readonly")
        self.processing_model_dropdown.pack(fill=tk.X, pady=(0, 10))
        ...
        self.gemini_settings_frame.pack(fill=tk.X)
        ttk.Label(self.gemini_settings_frame, text="API Key File:").pack(fill=tk.X, pady=(0, 2))
        api_frame = ttk.Frame(self.gemini_settings_frame); api_frame.pack(fill=tk.X)
        ...
        ttk.Label(self.gemini_settings_frame, text="Model Name:").pack(fill=tk.X, pady=(10, 2))
        ttk.Entry(self.gemini_settings_frame, textvariable=self.model_name_var).pack(fill=tk.X)
        ttk.Label(self.gemini_settings_frame, text="Temperature:").pack(fill=tk.X, pady=(10, 2))
        ttk.Scale(self.gemini_settings_frame, from_=0.0, to=1.0, orient=tk.HORIZONTAL, variable=self.temperature_var).pack(fill=tk.X)
        ttk.Label(self.gemini_settings_frame, text="Max Output Tokens:").pack(fill=tk.X, pady=(10, 2))
        ...
        self.ollama_settings_frame.pack(fill=tk.X, pady=10)
    ...
    def load_initial_settings(self):
        ...
        self.api_key_file_var.set(api_cfg.get('key_file', 'gemini_api.key'))
        self.model_name_var.set(llm_cfg.get('model_name', 'gemini-1.5-flash'))
    ...
    def _fetch_ollama_models(self) -> list[str]:
        """Fetches a list of installed models from the Ollama API."""
        try:
            logging.info("Attempting to fetch models from Ollama API at http://localhost:11434...")
            response = requests.get("http://localhost:11434/api/tags", timeout=3)
            response.raise_for_status()
            models_data = response.json()
            model_names = [model['name'] for model in models_data.get('models', [])]
            if model_names:
                logging.info(f"Successfully fetched {len(model_names)} models from Ollama.")
            else:
                logging.warning("Ollama API responded, but no models are installed.")
            return sorted(model_names)
        except requests.exceptions.ConnectionError:
            logging.error("Could not connect to Ollama server. Is Ollama running?")
            return []
        except Exception as e:
            logging.error(f"An error occurred while fetching Ollama models: {e}")
            return []

    def populate_ollama_models_dropdown(self):
        """Fetches Ollama models and populates the dropdown combobox."""
        ...
        if model_list:
            ...
            self.ollama_model_var.set("[Ollama not running or no models found]")
    ...
    def run_full_process(self):
        """Main logic to dispatch the correct generation process."""
        try:
            mode = self.input_mode_var.get()
            if mode == "scrape":
                self.run_gemini_scrape_process()
            else: # "upload" mode
                processing_engine = self.processing_model_var.get()
                if processing_engine == "Ollama RAG":
                    self.run_ollama_rag_process()
                else: # "Gemini Summarization"
                    self.run_gemini_local_process()
            ...
            # Always re-enable the start button after a run
            self.start_button.config(state=tk.NORMAL)

    def run_gemini_scrape_process(self):
        logging.info("--- Starting in Web Scrape + Gemini Mode ---")
        ...
        self.generate_gemini_notes(api_key, source_data, source_name)


    def run_gemini_local_process(self):
        logging.info("--- Starting in Local Document + Gemini Mode ---")
    ...
    def run_ollama_rag_process(self):
        logging.info("--- Starting in Local Document + Ollama RAG Mode ---")
        file_paths = self.file_listbox.get(0, tk.END)
        if not file_paths: 
            messagebox.showerror("Input Error", "Please add at least one document for RAG."); return
            
        ollama_model = self.ollama_model_var.get()
        if not ollama_model or "not running" in ollama_model:
            messagebox.showerror("Input Error", "Please select a valid Ollama model from the dropdown."); return

        main_prompt = self.prompt_text.get("1.0", tk.END).strip()
        if not main_prompt:
            messagebox.showerror("Input Error", "The prompt cannot be empty."); return

        generator = OllamaRAGGenerator(file_paths, ollama_model)
        self.final_notes_content = generator.generate(main_prompt)
        
        if self.final_notes_content and not self.final_notes_content.startswith("#"):
            logging.info("--- Ollama RAG Generation Complete! ---")
            logging.info("Ready to save the file.")
            self.save_button.config(state=tk.NORMAL)
        else:
            logging.error("Ollama RAG generation failed. Check logs for details.")
            self.save_button.config(state=tk.DISABLED)


    def _get_gemini_api_key(self):
        api_key_file = self.api_key_file_var.get()
        if not api_key_file: messagebox.showerror("Input Error", "API Key File for Gemini must be set."); return None
        api_key = APIKeyManager.get_api_key(api_key_file)
        if not api_key: logging.error("Gemini API Key is invalid. Halting."); return None
        return api_key

    def generate_gemini_notes(self, api_key, source_data, source_name):
        model_name = self.model_name_var.get(); prompt = self.prompt_text.get("1.0", tk.END).strip()
        if not model_name or not prompt:
            messagebox.showerror("Input Error", "Gemini Model and Prompt must be set."); return
