...
# Make sure to install these: pip install beautifulsoup4 langchain-google-genai google-generativeai PyMuPDF python-docx
...
class ScraperApp:
    ...
    def _set_child_widgets_state(self, parent, state):
        """
        FIX: Recursively sets the state of all child widgets.
        This safely handles containers like Frames that don't have a 'state' option.
        """
        for widget in parent.winfo_children():
            try:
                widget.configure(state=state)
            except tk.TclError:
                # This widget does not have a 'state' option, so we ignore it
                pass
            self._set_child_widgets_state(widget, state)

    def toggle_input_mode(self):
        """Enables/disables GUI elements using the new robust helper method."""
