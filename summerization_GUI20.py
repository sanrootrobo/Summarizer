...
class MarkdownPreviewWidget(ttk.Frame):
    ...
    def create_preview_widgets(self):
        ...
        # The command is now handled by the main app to support playlists
    ...
    def open_in_browser(self, content_to_open=None):
        """Opens the provided content or the current preview content in a browser."""
        ...
        try:
            ...
            self.parent.after(5000, lambda: os.unlink(temp_path) if os.path.exists(temp_path) else None)
...
class APIKeyManager:
    ...
    def get_gemini_api_keys(filepath: str) -> List[str]:
        """Loads one or more API keys from a file."""
        if not Path(filepath).is_file():
            logging.error(f"Gemini API key file not found at {filepath}")
            return []
        try:
            ...
            if not keys:
                raise ValueError("Key file is empty or all lines are commented out.")
            logging.info(f"🔑 Loaded {len(keys)} Gemini API key(s).")
            return keys
        except Exception as e:
            logging.error(f"❌ Failed to load Gemini API keys: {e}")
            return []
...
class EnhancedNoteGenerator:
    ...
    def generate_comprehensive_notes(self, source_data: dict[str, str], source_name: str) -> str:
        ...
        # Use a regex to find the first H1 header as the title, fallback to a generic title
        title_match = re.search(r'^#\s+(.*)', organized_content, re.MULTILINE)
        document_title = title_match.group(1).strip() if title_match else source_name
        ...
        try:
            ...
            # Ensure the output starts with a clear title
            if not notes.strip().startswith("#"):
                notes = f"# {document_title}\n\n{notes}"
...
class AdvancedScraperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Enhanced Research & Study Guide Generator v2.8 (Auto-Save)")
    ...
    def create_source_tab(self, parent):
        ...
        ttk.Radiobutton(mode_frame, text="🎬 YouTube Video/Playlist", variable=self.input_mode_var, value="youtube_video", command=self.toggle_input_mode).pack(anchor=tk.W)
    ...
    def create_output_tab(self, parent):
        ...
        hugo_frame.pack(fill=tk.X, pady=(0, 10))
    ...
    def run_full_process(self):
        try:
            ...
            if not self.loaded_api_keys:
                messagebox.showerror("API Key Error", "The Gemini API key is missing or invalid."); return
            api_key = self.loaded_api_keys[0] # Use the first key for serial tasks
            ...
            else: self.root.after(0, lambda: self.progress_var.set("Complete! Ready for next step"))
    
    def process_single_video(self, video_url: str, api_key: str, temp_dir: Path) -> Optional[str]:
        """Core logic to process one video. Returns the generated markdown content."""
        try:
            downloaded_video_filename = temp_dir / f"video_{random.randint(1000, 9999)}.mp4"
            logging.info(f"Downloading and converting video: {video_url}")
            yt_dlp_command = ['yt-dlp', '-f', 'bestvideo[height<=480]+bestaudio/best[height<=480]', '--recode-video', 'mp4', '-o', str(downloaded_video_filename), '--force-overwrite', video_url]
            try:
                subprocess.run(yt_dlp_command, check=True, capture_output=True, text=True)
            except subprocess.CalledProcessError as e:
                logging.error(f"yt-dlp failed for {video_url}. Stderr: {e.stderr}")
                return f"# Download Error\n\nFailed to download video: `{video_url}`. See logs."
            
            if not downloaded_video_filename.exists():
                logging.error(f"Download failed for {video_url}, file not created.")
                return f"# File Error\n\nCould not find downloaded file for video: `{video_url}`."

            logging.info(f"✅ Download complete. Uploading to API...")
            genai.configure(api_key=api_key)
            video_file = genai.upload_file(path=str(downloaded_video_filename))
            ...
            if video_file.state.name == "FAILED":
                logging.error(f"API processing failed for {video_url}.")
                return f"# API Error\n\nGoogle Gemini API failed to process video: `{video_url}`."

            logging.info(f"✅ API processing complete for {video_url}. Generating notes...")
            ...
            logging.error(f"❌ Unexpected error processing video {video_url}: {e}", exc_info=True)
            return f"# UNEXPECTED ERROR\n\nAn error occurred while processing `{video_url}`:\n\n```\n{e}\n```"

    def run_youtube_video_analysis(self):
        logging.info("🚀 Starting YouTube Video Analysis...")
        self.progress_var.set("Validating inputs...")
        video_url = self.url_var.get().strip()
        user_prompt = self.prompt_text.get("1.0", tk.END).strip()

        if not all([video_url, self.loaded_api_keys, user_prompt]):
            messagebox.showerror("Input Error", "YouTube URL, a valid API Key file, and a prompt are required.")
            return

        if not shutil.which('yt-dlp'):
            logging.error("❌ 'yt-dlp' command not found.")
            messagebox.showerror("Missing Tool", "'yt-dlp' is required for this feature.")
            return

        temp_dir = Path(tempfile.mkdtemp(prefix="video_analysis_"))
        try:
            self.progress_var.set("Processing video...")
            # Use the first key for single video analysis
            self.final_notes_content = self.process_single_video(video_url, self.loaded_api_keys[0], temp_dir)
    ...
    def run_youtube_playlist_analysis(self, playlist_url):
        logging.info(f"🚀 Starting YouTube Playlist Analysis for: {playlist_url}")
        self.progress_var.set("Fetching playlist videos...")
        
        if not shutil.which('yt-dlp'):
            messagebox.showerror("Missing Tool", "'yt-dlp' is required for playlist analysis.")
            return
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                videos_to_process = [(entry['url'], entry.get('title', f"video-{entry['id']}")) for entry in info.get('entries', []) if entry]
        except Exception as e:
            messagebox.showerror("Playlist Error", f"Could not fetch playlist videos: {e}")
            logging.error(f"❌ Failed to get playlist info: {e}")
            return
        
        if not videos_to_process:
            messagebox.showinfo("No Videos", "Could not find any videos in the provided playlist URL.")
            return

        output_dir_str = self.output_dir_var.get()
        if self.auto_save_var.get() and output_dir_str:
            output_dir = Path(output_dir_str)
        else:
            output_dir_str = filedialog.askdirectory(title="Select a Directory to Save Playlist Study Guides")
            if not output_dir_str:
                logging.warning("User cancelled directory selection for playlist.")
                return
            output_dir = Path(output_dir_str)
        ...
        logging.info(f"Found {num_videos} videos. Processing in parallel with {num_keys} API key(s).")
        
        api_key_cycle = cycle(self.loaded_api_keys)
        processed_files = []
        
        temp_dir = Path(tempfile.mkdtemp(prefix="playlist_analysis_"))
        try:
            with ThreadPoolExecutor(max_workers=num_keys) as executor:
                future_to_video = {
                    executor.submit(self.process_single_video, video_url, next(api_key_cycle), temp_dir): (video_url, title)
                    for video_url, title in videos_to_process
                }
                
                for i, future in enumerate(as_completed(future_to_video), 1):
                    video_url, title = future_to_video[future]
                    self.progress_var.set(f"Processing {i}/{num_videos}: {title[:40]}...")
                    try:
                        ...
                        if markdown_content:
                            ...
                            logging.warning(f"⚠️ No content generated for {title}")
                    except Exception as e:
                        logging.error(f"❌ Failed to process video '{title}': {e}")
            
            self.last_generated_files = processed_files
            self.final_notes_content = f"# Playlist Processing Complete\n\nSuccessfully processed {len(processed_files)} out of {num_videos} videos.\n\nFiles saved in:\n`{output_dir}`"
            self.root.after(0, lambda: self.markdown_preview.update_preview(self.final_notes_content))
            self.progress_var.set(f"Complete! Saved {len(processed_files)} files.")
    ...
    def run_multimodal_upload_analysis(self):
        ...
        if not all([file_paths, self.loaded_api_keys, user_prompt]):
            messagebox.showerror("Input Error", "Please add at least one file, a valid API Key file, and a prompt.")
            return

        api_key = self.loaded_api_keys[0] # Use the first key for this task
    ...
    def toggle_input_mode(self):
        ...
        if mode == "scrape":
            ...
            self.url_input_frame.config(text="🎬 YouTube Video / Playlist URL")
    ...
    def update_api_key_status(self, *args):
        ...
        if not filepath:
            ...
            self.loaded_api_keys = APIKeyManager.get_gemini_api_keys(filepath)
        
        num_keys = len(self.loaded_api_keys)
        if num_keys == 0:
            self.api_key_status_label.config(text="No valid keys found in file.", foreground=DARK_THEME['error'])
        elif num_keys == 1:
            self.api_key_status_label.config(text="✅ Loaded 1 API Key.", foreground=DARK_THEME['success'])
        else:
            self.api_key_status_label.config(text=f"✅ Loaded {num_keys} API Keys for parallel processing.", foreground=DARK_THEME['success'])
    ...
    def toggle_auto_save_panel(self):
        is_enabled = self.auto_save_var.get()
        new_state = tk.NORMAL if is_enabled else tk.DISABLED
        self.output_dir_entry.config(state=new_state)
        self.output_dir_button.config(state=new_state)
    ...
    def load_initial_settings(self):
        ...
        if self.config:
            ...
            hugo_settings = output_settings.get('hugo', {})
            ...
            self.output_dir_var.set(output_settings.get('directory', ''))

            self.hugo_enabled_var.set(hugo_settings.get('enabled', False))
            self.hugo_dir_var.set(hugo_settings.get('directory', ''))
            self.hugo_content_dir_var.set(hugo_settings.get('content_directory', 'posts'))
        ...
        self.update_api_key_status() # Trigger initial check
    ...
    def save_notes(self):
        ...
        source_name = self.sanitize_filename(source_name_raw)
    ...
    def start_task_thread(self):
        ...
        if not self.loaded_api_keys:
            messagebox.showerror("API Key Error", "Cannot start. Please provide a valid Gemini API key file.")
            return
        ...
        self.final_notes_content = ""
        self.last_generated_files = [] # Reset file list for new job
        ...
        if mode == "youtube_video":
            ...
            if 'list' in query_params:
                target_function = self.run_youtube_playlist_analysis
                args = (url,)
            else:
    ...
    def _finalize_generation(self):
        """Called after content generation is complete to decide the next step."""
        if self.auto_save_var.get() and self.output_dir_var.get():
            self._perform_auto_save()
        elif self.hugo_enabled_var.get():
            self.handle_hugo_integration()
        else:
            # Enable manual save if no automatic action is taken
    ...
    def _perform_auto_save(self):
        """Saves the generated content to the specified output directory."""
        output_dir_str = self.output_dir_var.get()
        if not output_dir_str:
            logging.warning("⚠️ Auto-save enabled but no directory specified. Skipping.")
            self.save_button.config(state=tk.NORMAL) # Allow manual save as fallback
            return
        ...
        try:
            ...
            source_name_raw = "video_url" if self.input_mode_var.get() == "youtube_video" else (urlparse(self.url_var.get()).netloc or "website" if self.input_mode_var.get() == "scrape" else "local_files")
            source_name = self.sanitize_filename(source_name_raw)
            filename = f"study_guide_{source_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
            filepath = output_dir / filename

            with open(filepath, "w", encoding="utf-8") as f:
                f.write(self.final_notes_content)
            
            self.last_generated_files = [str(filepath)] # Store the saved file path
            logging.info(f"✅ Automatically saved study guide to: {filepath}")
            self.progress_var.set(f"Complete! Saved to {output_dir.name}")

        except Exception as e:
            logging.error(f"❌ Auto-save failed: {e}")
            messagebox.showerror("Auto-Save Error", f"Could not automatically save file:\n{e}")
            self.save_button.config(state=tk.NORMAL) # Allow manual save on failure
    ...
    def sanitize_filename(self, text: str) -> str:
        ...
        if not text: return ""
        text = text.lower()
        text = re.sub(r'[\s\.]+', '-', text) # Replace spaces and dots with a hyphen
        text = re.sub(r'[^a-z0-9-_]', '', text) # Remove all non-alphanumeric/hyphen/underscore chars
        return text.strip('-_')[:100] # Limit length

    def handle_hugo_integration(self):
        """Creates a new post in a Hugo site with the generated content."""
        self.progress_var.set("Integrating with Hugo...")
        logging.info("🔩 Starting Hugo integration...")
        hugo_root_str = self.hugo_dir_var.get()
        if not hugo_root_str:
            messagebox.showerror("Hugo Error", "Hugo project root directory is not specified.")
            self.save_button.config(state=tk.NORMAL) # Allow manual save
            return

        hugo_root = Path(hugo_root_str)
        if not hugo_root.is_dir() or not (hugo_root / "content").is_dir():
            messagebox.showerror("Hugo Error", f"Invalid Hugo directory:\n'{hugo_root_str}'\n\nPlease select a valid Hugo project root containing a 'content' folder.")
            self.save_button.config(state=tk.NORMAL)
            return

        if not shutil.which("hugo"):
            messagebox.showerror("Hugo Error", "'hugo' command not found in your system's PATH.\nPlease install Hugo or correct your PATH.")
            self.save_button.config(state=tk.NORMAL)
            return

        try:
            # The content passed to hugo should not have the metadata block
            content_only = re.sub(r'^GENERATION METADATA:.*^=+\n\n', '', self.final_notes_content, flags=re.DOTALL | re.MULTILINE)
            title_match = re.search(r'^#\s+(.*)', content_only, re.MULTILINE)
            if title_match:
                title = title_match.group(1).strip()
            else:
                title = f"Generated Study Guide {datetime.now().strftime('%Y-%m-%d %H:%M')}"
                content_only = f"# {title}\n\n{content_only}" # Add title if missing

            sanitized_title = self.sanitize_filename(title)
            if not sanitized_title: # Handle cases where title is only symbols
                sanitized_title = f"post-{datetime.now().strftime('%Y%m%d%H%M%S')}"
            
            # Use the user-defined content subdirectory, defaulting to 'posts'
            content_subdir = self.hugo_content_dir_var.get().strip('/\\') or 'posts'
            post_rel_path = f"{content_subdir}/{sanitized_title}.md"
            
            # This is the full path for checking existence, not for the command
            full_post_path = hugo_root / "content" / post_rel_path

            if full_post_path.exists():
                if not messagebox.askyesno("File Exists", f"A post already exists at:\n{full_post_path}\n\nDo you want to overwrite it?"):
                    logging.warning("Hugo integration cancelled by user due to existing file.")
                    self.save_button.config(state=tk.NORMAL)
                    return
                else:
                    logging.info(f"User chose to overwrite existing Hugo post: {full_post_path}")

            logging.info(f"Running Hugo command to create new post: {post_rel_path}")
            hugo_executable = shutil.which("hugo")
            # CORRECTED COMMAND: Pass path relative to 'content' dir, not including 'content'
            cmd = [hugo_executable, 'new', post_rel_path.replace(os.path.sep, '/')]
            process = subprocess.run(cmd, cwd=hugo_root, check=True, capture_output=True, text=True, encoding='utf-8')
            
            # Hugo's output gives the full path to the created file
            created_file_path_str = process.stdout.strip().split(" ")[0]
            created_file_path = Path(created_file_path_str)

            if not created_file_path.exists():
                raise FileNotFoundError(f"Hugo reported creating a file, but it was not found at {created_file_path}")

            logging.info(f"Appending generated content to {created_file_path}")
            with open(created_file_path, "a", encoding="utf-8") as f:
                f.write("\n" + content_only)

            self.progress_var.set("Hugo post created!")
            logging.info(f"✅ Successfully created and updated Hugo post: {created_file_path}")
            messagebox.showinfo("Hugo Success", f"Successfully created new Hugo post:\n\n{created_file_path}")

        except (subprocess.CalledProcessError, FileNotFoundError, IOError) as e:
            logging.error(f"❌ Hugo integration failed: {e}")
            if isinstance(e, subprocess.CalledProcessError):
                error_msg = f"Hugo command failed:\n\n{e.stderr}"
            else:
                error_msg = f"An error occurred during Hugo integration:\n\n{e}"
            messagebox.showerror("Hugo Integration Error", error_msg)
            self.save_button.config(state=tk.NORMAL) # Allow manual save on failure

    def prompt_and_open_in_browser(self):
        """Smartly handles opening in browser for single files or playlists."""
        if not self.last_generated_files:
            # Fallback to current preview if no files were tracked
            self.markdown_preview.open_in_browser()
            return

        if len(self.last_generated_files) == 1:
            try:
                content = Path(self.last_generated_files[0]).read_text(encoding='utf-8')
                self.markdown_preview.open_in_browser(content_to_open=content)
            except Exception as e:
                messagebox.showerror("File Error", f"Could not read the generated file:\n{e}")
        else:
            # More than one file, so prompt the user
            dialog = tk.Toplevel(self.root)
            dialog.title("Select File to Preview")
            dialog.geometry("500x350")
            dialog.configure(bg=DARK_THEME['frame_bg'])
            
            ttk.Label(dialog, text="Multiple files were generated. Select one to open:", background=DARK_THEME['frame_bg']).pack(pady=10, padx=10)
            
            list_frame = ttk.Frame(dialog)
            list_frame.pack(pady=5, padx=10, fill=tk.BOTH, expand=True)

            listbox = tk.Listbox(list_frame, bg=DARK_THEME['text_bg'], fg=DARK_THEME['text_fg'], selectbackground=DARK_THEME['select_bg'], relief='flat')
            ...
            def on_open():
                ...
                selected_path = self.last_generated_files[selection[0]]
                try:
            ...
            ttk.Button(btn_frame, text="Open Preview", command=on_open).pack(side=tk.LEFT, padx=5)
